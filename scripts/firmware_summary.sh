...
# 只有存在 manifest 时，才能可靠判断请求包是否最终进入固件。
if [[ -n "$manifest_file" && -s "$manifest_file" && -s "$requested_pkgs" && -s "$all_pkgs" ]]; then
...
classify_pkg() {
  ...
  case "$pkg" in
    ...
    *)
      if [[ -s "$requested_pkgs" ]] && grep -Fxq "$pkg" "$requested_pkgs"; then
        echo "software"
      else
        echo ""
      fi
...
write_category "已安装固件" "$(category_file firmware)"
write_category "已安装驱动" "$(category_file drivers)"
write_category "已安装主题" "$(category_file themes)"
write_category "已安装 LuCI 插件" "$(category_file luci)"
write_category "已安装软件" "$(category_file software)"
write_category "第三方包编译失败/已跳过" "$third_party_failed_pkgs"
write_category "请求安装但未进入固件的包（安装失败/已跳过）" "$install_failed_pkgs"